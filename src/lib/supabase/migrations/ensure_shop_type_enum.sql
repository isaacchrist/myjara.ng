-- =========================================================
-- SAFE FIX: Ensure shop_type Enum has valid values & Fix Trigger
-- =========================================================

-- 1. Safely add values to the Enum (if it exists)
DO $$
BEGIN
    -- Check if the type exists
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'shop_type') THEN
        -- Add 'physical' if not present
        BEGIN
            ALTER TYPE public.shop_type ADD VALUE 'physical';
        EXCEPTION WHEN duplicate_object THEN END;
        
        -- Add 'online' if not present
        BEGIN
            ALTER TYPE public.shop_type ADD VALUE 'online';
        EXCEPTION WHEN duplicate_object THEN END;

        -- Add 'market_day' if not present
        BEGIN
            ALTER TYPE public.shop_type ADD VALUE 'market_day';
        EXCEPTION WHEN duplicate_object THEN END;
    ELSE
        -- If type doesn't exist, we assume it's TEXT, which is fine.
        NULL;
    END IF;
END $$;


-- 2. Update the Trigger to uses these values
...
BEGIN
  ...
  IF is_brand OR is_retailer THEN
    ...
    -- Insert with explicit string casting to avoid strict Enum issues (Postgres usually handles text->enum auto-cast)
