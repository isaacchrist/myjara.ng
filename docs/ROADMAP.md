...
**Status:** Phase 0 (0.1–0.5), Phase 1 (1.1–1.5), and Phase 2.1 + 2.2 + 2.4 + 2.6 have been implemented in code — see `supabase/migrations/021`–`031` and `supabase/migrations/README.md`. Migrations 021–024 (Phase 0) have been applied to the live database via the Supabase SQL Editor. Migrations 025–031 (Phase 1, 2.1, 2.2, 2.6) are written but **not yet applied** — same process, run them in order via the SQL Editor. Phase 2.6 also needs a `CRON_SECRET` env var set in the Vercel project (Vercel Cron sends it automatically as a Bearer header once configured) for `/api/cron/subscription-reminders` to run. **Phase 2.2 needs a decision — see the open question below — before its subdomain routing can be trusted in production.** Remaining Phase 2 items (2.3, 2.5) and Phases 3–4 are unstarted. A real `dialog.tsx` primitive now exists (`src/components/ui/dialog.tsx`, built for 2.4) — Phase 4's glassmorphism item can reuse it instead of building from scratch.
