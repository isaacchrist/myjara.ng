...
**Status:** Phase 0 (0.1–0.5), Phase 1 (1.1–1.5), and Phase 2.1 + 2.2 + 2.4 + 2.6 have been implemented in code, and 2.3's plumbing (dead-code cleanup, shared subscription UI, wholesaler paywall gate) is done with only pricing decisions left — see `supabase/migrations/021`–`031` and `supabase/migrations/README.md`. Migrations 021–024 (Phase 0) have been applied to the live database via the Supabase SQL Editor. Migrations 025–031 (Phase 1, 2.1, 2.2, 2.6) are written but **not yet applied** — same process, run them in order via the SQL Editor. Phase 2.6 also needs a `CRON_SECRET` env var set in the Vercel project (Vercel Cron sends it automatically as a Bearer header once configured) for `/api/cron/subscription-reminders` to run. **Phase 2.2 needs a decision — see the open question below — before its subdomain routing can be trusted in production. Phase 2.3's remaining pricing decisions are blocking too.** Remaining Phase 2 item (2.5) and Phases 3–4 are unstarted. A real `dialog.tsx` primitive now exists (`src/components/ui/dialog.tsx`, built for 2.4) — Phase 4's glassmorphism item can reuse it instead of building from scratch.
...
- **Real pre-existing bug fixed in the same file:** subdomain routing (`brandname.<domain>`) checked `hostname.includes('myjara.com')`, but every other functional link in the codebase (order/subscription emails, action links) uses `myjara.ng` — on the real production domain, subdomain routing was silently never triggering. Standardized on `myjara.ng` as canonical since it's the dominant convention across working links; **see the open question below, this needs confirmation.**

**2.3 Differentiated pricing & a real paywall — mechanical parts done, pricing/billing-model parts blocked on a decision (see open questions)**
...
- **Not done, blocked:** splitting retailer `SUBSCRIPTION_PLANS` by `shop_type` (physical/online/market_day) and confirming the wholesaler billing model both require real pricing decisions, not just plumbing — see open questions below. Note `WHOLESALER_PLANS` (flat ₦15,000/₦45,000 tiers) already exists and is now fully wired end-to-end; the roadmap's original "per-completed-order conversion" idea may already be moot if flat tiers are the intended model — needs confirmation either way.
...
- **Phase 2.2:** the codebase uses three different domain strings for the platform (`myjara.ng` in emails/action links, `myjara.com.ng` in `robots.ts`/`sitemap.ts`, `myjara.com` previously in the subdomain middleware). Subdomain and custom-domain routing now hardcode `myjara.ng` as canonical (`src/lib/supabase/middleware.ts`) since it's the majority/functional convention — confirm this is the actual registered+deployed domain, or subdomain routing will silently fail again on whatever domain is actually live.
- **Phase 2.3 (blocking):** two real pricing decisions, not engineering — (1) should wholesalers stay on the existing flat `WHOLESALER_PLANS` tiers (₦15,000 Supplier Basic / ₦45,000 Supplier Pro, both already fully wired end-to-end as of this pass) or move to a per-completed-order conversion model as the roadmap originally speculated? (2) should retailer `SUBSCRIPTION_PLANS` differ by `shop_type` (physical/online/market_day), and if so what should the actual tiers/prices be? Both need real numbers from whoever owns pricing, not a guess.
