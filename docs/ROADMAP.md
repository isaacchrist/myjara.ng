...
**Status:** Phase 0 (0.1–0.5), Phase 1 (1.1–1.5), and Phase 2.1 + 2.2 + 2.4 + 2.6 have been implemented in code, and 2.3 is now fully done (admin-editable pricing shipped — see below) except the wholesaler per-conversion billing engine, which is a separate build blocked on billing-mechanics answers — see `supabase/migrations/021`–`032` and `supabase/migrations/README.md`. Migrations 021–024 (Phase 0) have been applied to the live database via the Supabase SQL Editor. Migrations 025–032 (Phase 1, 2.1, 2.2, 2.3, 2.6) are written but **not yet applied** — same process, run them in order via the SQL Editor. Phase 2.6 also needs a `CRON_SECRET` env var set in the Vercel project (Vercel Cron sends it automatically as a Bearer header once configured) for `/api/cron/subscription-reminders` to run. Confirmed with the site owner: canonical domain is `myjara.com.ng` (swept across the codebase), retailer plans should differ by shop_type (now supported via `/admin/pricing`), and 2.5 (multi-shop) should proceed to build. **Still blocking: the wholesaler per-conversion billing mechanics (conversion definition, rate, collection method) — see open questions.** Remaining Phase 2 item (2.5, now scoped, see its section) and Phases 3–4 are unstarted. A real `dialog.tsx` primitive now exists (`src/components/ui/dialog.tsx`, built for 2.4) — Phase 4's glassmorphism item can reuse it instead of building from scratch.
...
- **Not done, flagged for later:** `/seller/support` submits to a `console.log` + fake delay, not a real backend (support tickets aren't persisted anywhere); the legacy `chat_conversations`/`chat_messages` tables are dead and safe to drop in a future migration.
...
**2.5 Physical retailer multi-shop — scoped, confirmed by the site owner to proceed to build; not yet built**
- **Two competing precedents already exist in the code, and only one should survive.** (A) `store_logistics` (`001_initial_schema.sql:101-112`) is already a working, multi-row-per-store child table (`location_name`, `city`, `delivery_fee`, `coverage_zones`) rendered on the storefront and chosen at checkout (`orders.logistics_option_id`) — this is ~90% of a `store_locations` table already, just scoped to fulfillment rather than full location identity. (B) A separate, *abandoned* scaffold (`src/lib/store-context.ts`, `store-switcher.tsx`, `SellerStoreProvider`) queries `stores` by `owner_id` without `.single()` and lets a user pick an "active store" via cookie — but the "create a second store" flow it depends on (`/onboarding/store`, referenced 5 times) **doesn't exist anywhere in `src/app`**, so no retailer can actually reach a second `stores` row today. Decision: build on (A)'s pattern via a new `store_locations` table; do not resume (B) — chat (`UNIQUE(user_id, store_id)`) and the public storefront (one page per `slug`) both want one identity per retailer, which (B) would fragment with no aggregation logic anywhere to fix that.
- **Proposed schema:** `store_locations` (`id`, `store_id` FK, `name`, `address`, `city`, `latitude`, `longitude`, `phone` nullable, `gallery_urls jsonb`, `is_primary`, `is_active`, `location_type` physical/market_day, `market_name` for market_day rows). Fold `store_logistics` into it or add `store_logistics.location_id → store_locations(id)` so delivery/pickup configs attach per location. Migration backfills one `is_primary` location per existing store from `stores.market_name/latitude/longitude/phone/gallery_urls`, which stay in place short-term as denormalized "primary location" fields (many read sites depend on them directly) — a full migration of every read site to `store_locations` is bigger than this phase should take in one pass.
- **Real bug to fix regardless of scope:** `updateProfile()`/`updateSellerProfileAction` (`profile.ts:111-129`) and `approveWholesalerAction`/`rejectWholesalerAction` (`verification.ts`) both write via `.eq('owner_id', ...)` with no store/location scoping — once multi-location ships, these would silently apply to *every* location an owner has at once. Needs a `store_id`/`location_id` param before multi-location goes live.
- Products stay tied to `store_id` only; a nullable `products.location_id` (meaning "available at all locations") can absorb the ad-hoc per-product `pickup_location` hack already in `attributes` (`seller/products/new/page.tsx`) that sellers/devs built because no formal per-location pickup point exists yet — a real signal this need is already leaking into the product model.
- Not yet built — this is the largest remaining Phase 2 item; implementation should be its own pass given the schema + several call-site changes involved.
...
- `dialog.tsx` (frosted-glass) already exists, built early for 2.4's onboarding flow — align `Sheet`/`Toast` to the same treatment so all popups look consistent (currently `Sheet` still uses a plain `bg-background`, and there is otherwise zero glassmorphism anywhere else in the app except one stray `backdrop-blur-sm` badge in the store gallery).
